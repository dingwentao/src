 ...
 **/
/*-------------------------------------------------------------------------*/
int FTI_Init(char* configFile, char* szConfigFile, MPI_Comm globalComm)
{
	SZ_Init(szConfigFile);
...
int FTI_Protect(int id, void* ptr, long count, FTIT_type type)
{
    ...
    FTI_Data[FTI_Exec.nbVar].ptr2 = ptr;    
...
//				printf("myRank=%d, (double*)(FTI_Data[%d].ptr)[0]=%lf\n", FTI_Topo.myRank, i, ((double*)(FTI_Data[i].ptr))[0]);
				...
				bytes = SZ_compress(SZ_DOUBLE, (double*)(FTI_Data[i].ptr), &outSize, 0, 0, 0, 0, FTI_Data[i].count);
				FTI_Data[i].size = outSize;
				FTI_Data[i].ptr2 = bytes;
				FTI_Exec.ckptSize += (8+FTI_Data[i].size);				
			}
...
//				printf("FTI_Data[%d].ptr[0]=%d, FTI_Data[i].eleSize=%d, FTI_Data[i].count=%d, \n", i, ((int*)FTI_Data[i].ptr)[0], FTI_Data[i].eleSize, FTI_Data[i].count);
			}	           
