...
PetscErrorCode KSPSolve_GMRES(KSP ksp)
{
	...
	/* Dingwen */
	//FTI Initilize
	char FTIConFile[] = "config.fti";
	char SZConFile[]  = "sz.config";
	FTI_Init(FTIConFile, SZConFile, MPI_COMM_WORLD);

	//Register FTI for ksp solution vector
	PetscScalar *array;
	PetscInt lsize;
	ierr = VecGetArray(ksp->vec_sol,&array);CHKERRQ(ierr);
	ierr = VecGetLocalSize(ksp->vec_sol,&lsize);CHKERRQ(ierr);

	//Protect
	FTI_Protect(0, &itcount, 1,  FTI_INTG);
	FTI_Protect(1, &(ksp->guess_zero), 1, FTI_INTG);
	FTI_Protect(2, &(ksp->its), 1, FTI_INTG);
	FTI_Protect(3, &(ksp->rnorm0), 1, FTI_DBLE);
	FTI_Protect(4, array, lsize, FTI_DBLE);
	/* Dingwen */
	...
	/* Dingwen */
	PetscReal normr, normb;
	ierr = VecNorm(ksp->vec_rhs, NORM_2, &normb);
	/* Dingwen */
	...
	while (!ksp->reason) {
		...
		/* Dingwen */
		ierr     = VecNorm(VEC_TEMP_MATOP, NORM_2, &normr);
		printf ("iteration = %d, relative residual = %.2g\n", itcount, normr/normb);
		/* Dingwen */
	...
	/* Dingwen */
	ierr = VecRestoreArray(ksp->vec_sol,&array);CHKERRQ(ierr);
	FTI_Finalize();
	/* Dingwen */
	PetscFunctionReturn(0);
}
