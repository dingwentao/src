                 ...
                 FTIT_injection* FTI_Inje)
{
	...
	compressorName = (int)iniparser_getint(ini, "Basic:Compressor", -1);
	if(compressorName == NO_COMPRESSOR)
		FTI_Print("compressor = no compressor", FTI_INFO);
	else if(compressorName == ZLIB_COMPRESSOR)
		FTI_Print("compressor = ZLIB_COMPRESSOR", FTI_INFO);
	else if(compressorName == ZFP_COMPRESSOR)
		FTI_Print("compressor = ZFP_COMPRESSOR", FTI_INFO);
	else if(compressorName == SZ_COMPRESSOR)
		FTI_Print("compressor = SZ_COMPRESSOR", FTI_INFO);
