                    ...
                    FTIT_dataset* FTI_Data)
{
   ...
   for (i = 0; i < FTI_Exec->nbVar; i++) {
      ...
      if(FTI_Data[i].ptr != FTI_Data[i].ptr2)
      {
			 ...
			 errno = 0;
			 written += fwrite(((char*)FTI_Data[i].ptr2) + written, 1, FTI_Data[i].size - written, fd); 
			 fwrite_errno = errno;
			}		  
	  }
	  else
      ...
      FTIT_topology* FTI_Topo, FTIT_dataset* FTI_Data)
{
   ...
   for (i = 0; i < FTI_Exec->nbVar; i++) {
       ...
       char *data_ptr = FTI_Data[i].ptr2;

       if(FTI_Data[i].ptr != FTI_Data[i].ptr2)
		{
			res = MPI_File_write_at(pfh, offset, &(FTI_Data[i].size), 1, MPI_LONG, MPI_STATUS_IGNORE);
			offset += 8;
        }
      ...
      FTIT_topology* FTI_Topo,FTIT_dataset* FTI_Data)
{
   ...
   for (i = 0; i < FTI_Exec->nbVar; i++) {
      // SIONlib write call
      res = sion_fwrite(FTI_Data[i].ptr2, FTI_Data[i].size, 1, sid);
